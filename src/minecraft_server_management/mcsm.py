...
@mcsm.command('setup')
@click.option('--override', '-o', 'override', help="Enables overriding pre-existing configurations.", is_flag=True)
@choice_option('--mod-loader', '-ml', help="The mod loader to use.", type=click.Choice(['Vanilla', 'Fabric']), callback=get_version)
@auto_complete_option('--version', '-v', help="The version.", required=False, callback=version_defined)
def setup(override, mod_loader, version):
    if os.path.exists('./conf.json'):
        if not override:
            override = click.confirm("Conf.json already found, override?", default=False)
        if not override:
            return
    click.echo('Setup')
